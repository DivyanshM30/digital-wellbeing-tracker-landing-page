    ...
    <section id="download" className="py-20 sm:py-32 bg-primary/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8" role="list">
          {platforms.map((item, index) => (
            <div
              key={index}
              role="listitem"
              className="p-6 rounded-xl border border-border bg-card hover:border-primary focus-within:ring-2 focus-within:ring-primary transition-colors"
            >
              <h3 className="font-bold text-lg mb-2">{item.os}</h3>
              <p className="text-sm text-foreground/60 mb-4">{item.method}</p>
              <code className="block bg-muted p-3 rounded text-xs font-mono text-foreground break-all" aria-label={`Installation command for ${item.os}`}>
                {item.code}
              </code>
            </div>
          ))}
        ...
        <div className="max-w-2xl mx-auto bg-card border border-border rounded-xl p-6 md:p-8">
          ...
          <pre className="bg-foreground/5 p-4 rounded text-xs overflow-x-auto text-foreground">
...
python main.py`}</code>
          ...
          <p className="text-sm text-foreground/60 mt-4">
            📖 For detailed setup instructions, visit the <a href="https://github.com/DivyanshM30/digital-wellbeing-tracker" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">GitHub repository</a>
          </p>
        </div>

        {/* System Requirements */}
        <div className="max-w-2xl mx-auto mt-8 p-6 rounded-xl bg-accent/5 border border-accent/20">
          <h4 className="font-bold mb-4">System Requirements</h4>
          <ul className="text-sm text-foreground/70 space-y-2">
            <li>✓ Python 3.8+ (for running from source)</li>
            <li>✓ Windows 7+, Linux (Ubuntu 20.04+), or macOS 10.14+</li>
            <li>✓ 50MB+ disk space</li>
            <li>✓ &lt;50MB RAM typical usage</li>
          </ul>
        </div>
      </div>
    </section>
